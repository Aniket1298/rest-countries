...
export default function Main(props) {
  ...
  useEffect(() => {
    ...
    fetch(url).then((response) =>
      response.text().then((text) => setData(JSON.parse(text)))
    );
    setFiltered(data);
  }, []);
  //
  ...
  function filterByName(name) {
    ...
    if (name) {
      setFiltered(
        data.filter((item) => item.name.toLowerCase().includes(name))
      );
    }
  }
  ...
  const dropmenu = regions.map((name) => (
    <a className="dropdown-item" onClick={() => filterByRegion(name)}>
      {name}
    </a>
  ));
  const alldata = (filtered.length == 0 ? data : filtered).map((d) => (
    <Country
      name={d.name}
      region={d.region}
        ...
        <div className="filter">
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
              <a className="dropdown-item" onClick={() => setAll()}>
                All
              </a>
              {dropmenu}
            </div>
